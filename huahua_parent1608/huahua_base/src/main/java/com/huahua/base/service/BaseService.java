    ...
    private Specification<Label> createSpecification(Label label){
        return new Specification<Label>() {
            ...
            public Predicate toPredicate(Root<Label> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
                ...
                if(StringUtils.isNotEmpty(label.getId())){
                    //where id= ?
                    predicatesList.add(cb.equal(root.get("id").as(String.class),label.getId()));
                }
                if(StringUtils.isNotEmpty(label.getLabelname())){// and labelname like “%？%”
                    predicatesList.add(cb.like(root.get("labelname").as(String.class),"%"+label.getLabelname()+"%"));
                }
                if (StringUtils.isNotEmpty(label.getState())){
                    predicatesList.add(cb.equal(root.get("state").as(String.class),label.getState()));
                }
                if (null!=predicatesList && predicatesList.size()>0){
